	...
	for (var i = 0; i < cards.length; i++) {
		...
		if (matches && matches.length > 1) {
			...
			if (card.actions) {
				...
				if (action) {
					var movedToFinished = (action.data.listAfter
						&& action.data.listBefore
						&& (action.data.listBefore.name !== action.data.listAfter.name)
						&& ((!finishLists.length && finishLists === action.data.listAfter.name) || finishLists.indexOf(action.data.listAfter.name) > -1 ));
				    
					var inFinished = action.data.list
						&& ((!finishLists.length && finishLists === action.data.list.name) || finishLists.indexOf(action.data.list.name) > -1);

					if (movedToFinished || inFinished) {
